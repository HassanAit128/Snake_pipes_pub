...
run_methylkit <- function(
  ...
  tryCatch(
    {
      dir.create(paste0(dir, "/", "METHYLATION/DMRs"))
      ...
      save(myDiff, DMRs, file = output)
      write.csv(DMRs, paste0(dir, "METHYLATION/DMRs/methylkit_DMRs.csv"), row.names = FALSE)
      ...
      perc <- ggplot(df_summary, aes(x = "", y = count, fill = diffexpressed)) +
        ...
        labs(fill = "", x = paste0("Total DMRs (pval < 0.05) : ", total_DMRs), y = "Number of Regions")
      ggsave(filename = paste0(dir, "METHYLATION/DMRs/methylkit_summary.png"), plot = perc, width = 6, height = 4)
      ...
      volcano <- ggplot(data = myDiff, aes(x = meth.diff, y = -log10(pvalue), col = diffexpressed)) +
        ...
        geom_point(size = 2) +
        theme_minimal()
      ggsave(filename = paste0(dir, "/METHYLATION/DMRs/methylkit_VolcanoPlot.png"), plot = volcano, width = 8, height = 8)
      message("--- Done ---")
    },
    error = function(e) {
      error_found <<- TRUE
      if (exists("myDiff")){
        save(myDiff, file = output)
      }
...
condition_1_samples <- design_matrix$SampleID[design_matrix$Condition == unique_conditions[1]]
condition_2_samples <- design_matrix$SampleID[design_matrix$Condition == unique_conditions[2]]
sample.id <- c(condition_1_samples, condition_2_samples)
